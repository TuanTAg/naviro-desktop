...
function optionalString(value: unknown): string | null | undefined {
  ...
  if (typeof value !== 'string') throw new Error('Optional workspace identifiers must be strings')
  return value
}
...
function parseRoot(value: unknown): NaviroWorkspaceRoot {
  if (!isRecord(value)) throw new Error('Workspace root must be an object')
...
export function parseNaviroWorkspace(value: unknown): NaviroWorkspace {
  ...
  if (!Array.isArray(value.roots)) throw new Error('Workspace roots must be an array')
  const roots = value.roots.map(parseRoot)
  const rootIds = new Set(roots.map((root) => root.id))
  if (rootIds.size !== roots.length) throw new Error('Workspace root ids must be unique')
  const rootPaths = new Set(
    roots.map((root) => workspaceRootIdentityKey(root.path, root.executionHostId))
  )
  if (rootPaths.size !== roots.length) throw new Error('Workspace root paths must be unique per host')
...
export function parseNaviroWorkspaceCatalog(value: unknown): NaviroWorkspaceCatalog {
  ...
  requireSchemaVersion(value)
  if (!Array.isArray(value.workspaces)) throw new Error('Workspace catalog must contain workspaces')
  const workspaces = value.workspaces.map(parseNaviroWorkspace)
  const workspaceIds = new Set(workspaces.map((workspace) => workspace.id))
  if (workspaceIds.size !== workspaces.length) throw new Error('Workspace ids must be unique')
